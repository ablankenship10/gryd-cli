...
function app(params, opts, cb) {
  if (!fs.existsSync(cwd + "/" + params[0])) {
    ...
    exec("cp -R " + helpers.PROJECT_INSTALL_LOCATION + "/apps/v1 " + params[0],
      {cwd: cwd + "/apps"}, function (error, stdout, stderr) {
        if (error) {
          console.log(stderr);
...
function resource(params, opts, cb) {
  ...
  var contPath = cwd + "/apps/"+params[0]+"/controllers/"+names.controller+".js",
    modPath = cwd + "/apps/"+params[0]+"/models/"+names.model+".js";

  if(!fs.existsSync(cwd + "/apps/"+params[0])){
